...
export interface P2PSignal {
    ...
    candidate?: RTCIceCandidateInit; // For backward compatibility
    candidates?: RTCIceCandidateInit[]; // For batched candidates
...
// --- P2P Stats & Monitoring Stubs (For Build Compatibility) ---
...
export function startClusterMonitoring(callback: (info: ClusterInfo) => void) {
    // Real monitoring is event-driven in HlsPlayer.tsx
    // This is a placeholder for future global swarm coordinator.
    // We return empty cleanup.
    return () => { };
}

export function updateSwarmHealth(stats: any) {
    // no-op for now
}

export function getOptimizedTrackers() {
    return [
