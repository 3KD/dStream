...
export function StreamSettings() {
    ...
    const [publicNodeUrl, setPublicNodeUrl] = useState("");
    ...
    useEffect(() => {
        if (myStream) {
            ...
            // Extract existing streaming URL if available
            const existingUrl = (myStream.metadata as any).streaming || "";
            // Only set if it's NOT localhost (since localhost is default/ignored)
            if (existingUrl && !existingUrl.includes("localhost")) {
                setPublicNodeUrl(existingUrl);
            }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const eventTags = [
                ...
                ['image', myStream?.metadata.image || ""], // Keep image if exists
                ['streaming', publicNodeUrl || (myStream?.metadata as any)?.streaming || ""], // Use explicit override, or fallback
                ['t', 'dstream'],
                ['t', 'dstream'],
                ...tagList.map(t => ['t', t])
            ];
        ...
        <div className="space-y-6">
            <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
                ...
                <div className="pt-2 border-t border-neutral-800 mt-2">
                    <label className="block text-sm font-bold text-blue-400 mb-2">Public Node URL (For Hosting)</label>
                    <div className="relative">
                        <Globe className="absolute left-3 top-3 w-5 h-5 text-neutral-600" />
                        <input
                            type="text"
                            value={publicNodeUrl}
                            onChange={e => setPublicNodeUrl(e.target.value)}
                            placeholder="https://my-tunnel.trycloudflare.com (Leave empty for default)"
                            className="w-full bg-neutral-950 border border-neutral-800 rounded-lg pl-10 pr-4 py-3 text-white focus:outline-none focus:border-blue-500 font-mono text-sm"
                        />
                    </div>
                    <p className="text-xs text-neutral-500 mt-2">
                        If running on a VPS or Tunnel, enter your public stream URL here (e.g., location of index.m3u8).
                        <br />
                        Default: <code>http://localhost:8880/STREAM_ID/index.m3u8</code>
                    </p>
                </div>
