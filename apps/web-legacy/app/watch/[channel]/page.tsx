...
import { P2PPlayer } from "@/components/p2p/P2PPlayer"; // Re-integrated
...
export default function WatchPage({ params }: WatchPageProps) {
    ...
    const [viewMode, setViewMode] = useState<'p2p' | 'hls'>('p2p'); // Default to P2P (Core Vision)
    ...
    // Direct HLS Source
    const streamUrl = `/api/hls/${channel}/index.m3u8`;
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        <div className="w-full relative">
                            {price && !hasTicket && !isReconnecting ? (
                                ...
                                viewMode === 'p2p' && activeStream?.pubkey ? (
                                    <div className="relative w-full h-full">
                                        <P2PPlayer
                                            broadcasterPubkey={activeStream.pubkey}
                                            onStatusChange={(s) => console.log("[P2P Status]", s)}
                                        />
                                        <button
                                            onClick={() => setViewMode('hls')}
                                            className="absolute top-4 right-4 bg-neutral-900/80 text-white px-3 py-1 rounded text-xs border border-white/20 z-20 hover:bg-neutral-800"
                                        >
                                            Switch to HLS (Backup)
                                        </button>
                                    </div>
                                ) : (
                                    <div className="relative w-full h-full">
                                        <VideoPlayer
                                            src={streamUrl}
                                            autoPlay={true}
                                            swarmId={activeStream?.pubkey || channel || undefined}
                                        />
                                        <button
                                            onClick={() => setViewMode('p2p')}
                                            className="absolute top-4 right-4 bg-purple-900/80 text-purple-200 px-3 py-1 rounded text-xs border border-purple-500/50 z-20 hover:bg-purple-900"
                                        >
