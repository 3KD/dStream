              ...
              {visibleStreams.map(stream => (
                ...
                >
                  <div className="aspect-video bg-neutral-800 flex items-center justify-center relative">
                    {stream.metadata?.image && (
                      /* eslint-disable-next-line @next/next/no-img-element */
                      <img
                        src={stream.metadata.image}
                        alt={stream.metadata.title || "Stream preview"}
                        className="absolute inset-0 w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity"
                      />
                    )}
                    <Play className="w-12 h-12 text-white/50 bg-black/50 rounded-full p-3 backdrop-blur-sm group-hover:scale-110 transition-transform z-10" />
                    <div className="absolute top-2 left-2 bg-red-600 text-white text-[10px] uppercase font-bold px-2 py-0.5 rounded z-10">
                      Live
                    </div>
                    {isTrusted(stream.pubkey) && (
                      <div className="absolute top-2 right-2 bg-green-900/80 text-green-400 p-1 rounded-full backdrop-blur-sm z-10">
