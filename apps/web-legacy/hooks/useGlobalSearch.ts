...
export function useGlobalSearch(query: string) {
    ...
    useEffect(() => {
        ...
        const combinedFilter: Filter = {
            kinds: [0, KIND_STREAM_ANNOUNCE],
            search: query,
            limit: 40
        } as any;

        let isMounted = true;
        // Cast filters to any to bypass strict Filter[] vs Filter mismatch in some nostr-tools versions
        const sub = pool.subscribeMany(RELAYS, combinedFilter as any, {
            onevent(event) {
                if (!isMounted) return;
