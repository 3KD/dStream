...
import { getPool, RELAYS, subscribeToEvents } from "@/lib/nostr";
...
export function InboxProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        ...
        // Define handleEvent first before using it
        const handleEvent = async (event: any) => {
            let peer = event.pubkey;
            let isSelf = event.pubkey === identity.publicKey;
            ...
            if (identity.privateKey) {
                decrypted = await decryptMessage(identity.privateKey, peer, event.content);
            }
        ...
        // nostr-tools v2 subscribeMany expects single filter, not array
        // We need two subscriptions: one for received messages, one for sent
        const unsubscribeReceived = subscribeToEvents(
            { kinds: [4], '#p': [identity.publicKey] },
            handleEvent,
            () => { }
        );

        const unsubscribeSent = subscribeToEvents(
            { kinds: [4], authors: [identity.publicKey] },
            handleEvent,
            () => { }
        );

        // Set loading false after a reasonable timeout
    ...
    const threadsWithStatus = threads
        ...
        .map(t => {
            ...
            // Safe check for t.messages and lastMsg being present
            if (!lastMsg) return { ...t, hasUnread: false };
