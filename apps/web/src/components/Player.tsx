...
export function Player({
  ...
  const [playbackMode, setPlaybackMode] = useState<"hls" | "whep" | "direct">("hls");
  ...
  useEffect(() => {
    ...
    const primaryWhepEndpoint = (whepSrc ?? "").trim();
    const secondaryWhepEndpoint = (fallbackWhepSrc ?? "").trim();
    const canFallbackSource = secondarySrc.length > 0 && secondarySrc !== primarySrc;
    let hlsFallbackAttempted = false;
    let whepFallbackAttempted = false;
    ...
    const startDirect = (source: string) => {
      setPlaybackMode("direct");
      setQualityOptions([]);
      setSelectedQuality(-1);
      setQualityIndicator("Native");
      setError(null);
      const onLoaded = () => {
        setStatus("Ready");
        sendReady();
      ...
      const onError = () => {
        setStatus("Offline");
        setError("Stream is offline or not available yet.");
      };
    ...
    const startHls = (source: string) => {
      setPlaybackMode("hls");
      let manifestNotReadyRetries = 0;
      let manifestRetryTimer: number | null = null;
      const clearManifestRetryTimer = () => {
        if (manifestRetryTimer !== null) {
          window.clearTimeout(manifestRetryTimer);
          manifestRetryTimer = null;
        }
      };
      const scheduleManifestRetry = (hls: Hls, delayMs = 750) => {
        if (manifestRetryTimer !== null) return;
        manifestRetryTimer = window.setTimeout(() => {
          manifestRetryTimer = null;
          if (cancelled) return;
          try {
            hls.startLoad();
      ...
      // Prefer native HLS (Safari is usually more reliable without hls.js),
      ...
      const integrityRewrite =
        integrityEnabled && source.includes("/api/dev/tamper-hls/")
      ...
      hls.on(Hls.Events.ERROR, (_event, data) => {
        ...
        const details = String((data as any)?.details ?? "");
        const manifestLike = details.includes("manifest") || details.includes("level");
        if (responseCode === 404 && manifestLike) {
          if (!hlsFallbackAttempted && canFallbackSource) {
            hlsFallbackAttempted = true;
            setError(null);
            setNeedsClick(false);
            setStatus("Loading…");
            setNote("Primary stream path unavailable (trying compatibility path).");
            clearManifestRetryTimer();
            hls.destroy();
            window.setTimeout(() => {
              if (cancelled) return;
              startSource(secondarySrc);
            }, 0);
            return;
          }
          manifestNotReadyRetries += 1;
          if (manifestNotReadyRetries <= 40) {
            setError(null);
            setNeedsClick(false);
            setStatus("Waiting for stream…");
            scheduleManifestRetry(hls);
            return;
          }
          clearManifestRetryTimer();
          setStatus("Offline");
    ...
    const startSource = (source: string) => {
    ...
    const tryWhep = async (endpoint: string) => {
      if (!endpoint) return false;
      setStatus("Loading…");
      try {
        const client = new WhepClient(endpoint);
        whepRef.current = client;
        const result = await client.start({ timeoutMs: 4500 });
        if (cancelled) {
          await client.close();
    ...
    if (integrityEnabled) {
      if (inferMediaUrlKind(primarySrc) === "direct") {
        setNote("Integrity verification unavailable for direct media source.");
        startDirect(primarySrc);
      } else {
        if (primaryWhepEndpoint) setNote("Integrity verification enabled (using HLS path).");
        startHls(primarySrc);
      }
    } else {
      void (async () => {
        const { mode, attemptedWhep } = await pickPlaybackMode({
          whepSrc: primaryWhepEndpoint,
          rtcSupported,
          tryWhep: () => tryWhep(primaryWhepEndpoint)
        });
        if (cancelled) return;
        if (mode === "whep") return;
        if (
          attemptedWhep &&
          !whepFallbackAttempted &&
          secondaryWhepEndpoint &&
          secondaryWhepEndpoint !== primaryWhepEndpoint
        ) {
          whepFallbackAttempted = true;
          const ok = await tryWhep(secondaryWhepEndpoint);
          if (cancelled) return;
          if (ok) {
            setNote("Using compatibility low-latency path.");
            return;
          }
        }
        if (attemptedWhep) setNote("Low-latency unavailable (falling back to HLS).");
        startSource(primarySrc);
      })();
    }
  ...
  }, [autoplayMuted, fallbackSrc, fallbackWhepSrc, integrity, lowLatencyEnabled, p2pSwarm, src, whepSrc]);
