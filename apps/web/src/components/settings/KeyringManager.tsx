...
import { Plus, Trash, ShieldCheck, Key } from "lucide-react";
import { useTrustedPeers } from "@/context/TrustedPeersContext";

export function KeyringManager() {
    const { trustedPubkeys, trust, untrust } = useTrustedPeers();
            ...
            <div className="bg-neutral-900 border border-neutral-800 rounded-xl p-6">
                ...
                <div className="flex gap-2 mb-6">
                    <div className="relative flex-1">
                        <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-500" aria-hidden="true" />
                        <input
                            type="text"
                            value={newKey}
                    ...
                    <button
                        ...
                        aria-label="Add to trusted keyring"
                    >
                ...
                {/* Key List */}
                <div className="space-y-2 max-h-64 overflow-y-auto" role="list" aria-label="Trusted keys">
                    {trustedPubkeys.length === 0 ? (
                        ...
                        trustedPubkeys.map((key: string) => (
                            <div key={key} role="listitem" className="flex items-center justify-between bg-neutral-950 p-3 rounded-lg border border-neutral-800 group hover:border-neutral-700 transition-colors">
                                ...
                                <button
                                    ...
                                    aria-label={`Remove key ${key.substring(0, 8)}...`}
                                >
                ...
                <div className="mt-4 text-xs text-neutral-600 flex justify-between">
                    <span>Total Trusted: {trustedPubkeys.length}</span>
