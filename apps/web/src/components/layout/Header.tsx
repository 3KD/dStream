...
export function Header({ showBroadcastButton = true, children }: HeaderProps) {
    ...
    const { session } = useBroadcast(); // removed isLive from destructuring if not available, checked types later. 
    // Legacy had isLive. BroadcastContext usually has session. If session is not null, isLive is true basically.
    // I'll check if isLive is exported. If not, derived from session?.status === 'live'.
                ...
                <div className="max-w-7xl mx-auto flex items-center justify-between">
                    ...
                    {isLive && (
                        <Link
                            ...
                            title={`Live: ${session?.stream.id || 'Stream'}`}
                        >
                    ...
                    <div className="flex gap-2 md:gap-4 items-center">
                        ...
                        <button
                            type="button"
                            onClick={() => setMobileMenuPath((value) => (value === pathname ? null : pathname))}
            ...
            {mobileMenuOpen && (
                <div className="md:hidden fixed top-[65px] left-0 right-0 bg-neutral-950/95 backdrop-blur-md border-b border-neutral-800 z-40 animate-in slide-in-from-top-2 duration-200">
                    <div className="max-w-7xl mx-auto p-4 flex flex-col gap-2">
                        ...
                        <button
                            type="button"
                            onClick={() => { setShowKeyring(true); setMobileMenuPath(null); }}
                        ...
                        <button
                            type="button"
                            onClick={() => { openInbox(); setMobileMenuPath(null); }}
                        ...
                        <button
                            type="button"
                            onClick={() => { setShowProfileEditor(true); setMobileMenuPath(null); }}
                            className="flex items-center gap-3 p-3 rounded-lg transition-colors text-neutral-400 hover:text-white hover:bg-neutral-800 text-left"
                        >
                            <UserCog className="w-5 h-5" />
