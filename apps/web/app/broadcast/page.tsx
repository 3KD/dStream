...
import { buildStreamAnnounceEvent, type StreamCaptionTrack, type StreamRendition } from "@dstream/protocol";
...
export default function BroadcastPage() {
  ...
  const [vodMode, setVodMode] = useState<"off" | "public" | "paid">("public");
  const [vodAccessScope, setVodAccessScope] = useState<"stream" | "playlist">("stream");
  const [vodPriceXmr, setVodPriceXmr] = useState("");
  const [vodAccessHours, setVodAccessHours] = useState("");
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (parsed.vodMode === "off" || parsed.vodMode === "public" || parsed.vodMode === "paid") setVodMode(parsed.vodMode);
      if (parsed.vodAccessScope === "stream" || parsed.vodAccessScope === "playlist") setVodAccessScope(parsed.vodAccessScope);
      if (typeof parsed.vodPriceXmr === "string") setVodPriceXmr(parsed.vodPriceXmr);
      if (typeof parsed.vodAccessHours === "string") setVodAccessHours(parsed.vodAccessHours);
      if (typeof parsed.vodPlaylistId === "string") setVodPlaylistId(parsed.vodPlaylistId);
  ...
  const vodPriceAtomic = useMemo(() => {
    const parsed = parseXmrAmountToAtomic(vodPriceXmr);
    if (parsed === null || parsed <= 0n) return undefined;
    return parsed.toString();
  }, [vodPriceXmr]);
  const vodPriceInvalid = useMemo(() => {
    if (vodMode !== "paid") return false;
    const trimmed = vodPriceXmr.trim();
    if (!trimmed) return true;
    return parseXmrAmountToAtomic(trimmed) === null;
  }, [vodMode, vodPriceXmr]);
  const vodAccessHoursParsed = useMemo(() => parsePositiveInt(vodAccessHours), [vodAccessHours]);
  const vodAccessHoursInvalid = useMemo(
    () => vodAccessHours.trim() !== "" && vodAccessHoursParsed === null,
    [vodAccessHours, vodAccessHoursParsed]
  );
  const vodAccessSeconds = useMemo(
    () => (vodAccessHoursParsed ? vodAccessHoursParsed * 3600 : undefined),
    [vodAccessHoursParsed]
  );
  const vodPlaylistIdTrimmed = useMemo(() => vodPlaylistId.trim(), [vodPlaylistId]);
  const vodAccessScopeInvalid = useMemo(
    () => vodMode === "paid" && vodAccessScope === "playlist" && !vodPlaylistIdTrimmed,
    [vodAccessScope, vodMode, vodPlaylistIdTrimmed]
  );
  ...
  const announce = useCallback(async (nextStatus: "live" | "ended") => {
    ...
    if (vodAccessHoursInvalid) throw new Error("Invalid VOD access window (use a positive integer number of hours).");
    if (vodMode === "paid" && vodPriceInvalid) {
      throw new Error("Invalid VOD price (use a number with up to 12 decimals).");
    }
    if (vodAccessScopeInvalid) {
      throw new Error("Playlist unlock scope requires a Playlist ID.");
    }
    ...
    const unsigned: any = buildStreamAnnounceEvent({
      ...
      vod: {
        mode: vodMode,
        priceAtomic: vodMode === "paid" ? vodPriceAtomic : undefined,
        currency: vodMode === "paid" ? "xmr" : undefined,
        accessSeconds: vodAccessSeconds,
        playlistId: vodPlaylistIdTrimmed || undefined,
        accessScope: vodMode === "paid" ? vodAccessScope : undefined
      },
    ...
    title,
    topics,
    vodAccessHoursInvalid,
    vodAccessScope,
    vodAccessScopeInvalid,
    vodAccessSeconds,
    vodMode,
    vodPlaylistIdTrimmed,
    vodPriceAtomic,
    vodPriceInvalid,
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="space-y-2">
                  <label className="text-xs text-neutral-400">VOD access mode</label>
                  <select
                    value={vodMode}
                    onChange={(e) => setVodMode(e.target.value as "off" | "public" | "paid")}
                    className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-2 text-sm focus:border-blue-500 focus:outline-none"
                    disabled={status === "connecting"}
                  >
                    <option value="public">Public replay</option>
                    <option value="paid">Paid replay (XMR)</option>
                    <option value="off">No replay (live only)</option>
                  </select>
                  <div className="text-xs text-neutral-500">Published in stream announce metadata for client-side discovery.</div>
                </div>
                <div className="space-y-2">
                  <label className="text-xs text-neutral-400">Paid unlock scope</label>
                  <select
                    value={vodAccessScope}
                    onChange={(e) => setVodAccessScope(e.target.value as "stream" | "playlist")}
                    className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-2 text-sm focus:border-blue-500 focus:outline-none"
                    disabled={status === "connecting" || vodMode !== "paid"}
                  >
                    <option value="stream">Per stream purchase</option>
                    <option value="playlist">Playlist bundle purchase</option>
                  </select>
                  {vodMode === "paid" ? (
                    <div className="text-xs text-neutral-500">
                      {vodAccessScope === "playlist"
                        ? "One paid unlock grants replay access to all videos in the same Playlist ID."
                        : "Unlock applies only to this single stream replay."}
                    </div>
                  ) : (
                    <div className="text-xs text-neutral-500">Only used for paid replay mode.</div>
                  )}
                </div>
                <div className="space-y-2">
                  <label className="text-xs text-neutral-400">VOD price (XMR, paid mode)</label>
                  <input
                    value={vodPriceXmr}
                    onChange={(e) => setVodPriceXmr(e.target.value)}
                    className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-2 text-sm focus:border-blue-500 focus:outline-none font-mono"
                    disabled={status === "connecting" || vodMode !== "paid"}
                    placeholder="0.10"
                  />
                  {vodPriceInvalid ? (
                    <div className="text-xs text-red-300">Paid mode requires a valid amount (up to 12 decimals).</div>
                  ) : (
                    <div className="text-xs text-neutral-500">Used by paid VOD unlock flow on watch pages.</div>
                  )}
                </div>
                <div className="space-y-2">
                  <label className="text-xs text-neutral-400">Access window (hours, optional)</label>
                  <input
                    value={vodAccessHours}
                    onChange={(e) => setVodAccessHours(e.target.value)}
                    className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-2 text-sm focus:border-blue-500 focus:outline-none font-mono"
                    disabled={status === "connecting"}
                    placeholder="24"
                  />
                  {vodAccessHoursInvalid ? (
                    <div className="text-xs text-red-300">Use a positive integer (hours).</div>
                  ) : (
                    <div className="text-xs text-neutral-500">Leave empty for unlimited access duration.</div>
                  )}
                </div>
              </div>

              <div className="space-y-2">
                <label className="text-xs text-neutral-400">Playlist ID (optional)</label>
                <input
                  value={vodPlaylistId}
                  onChange={(e) => setVodPlaylistId(e.target.value)}
                  className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-4 py-2 text-sm focus:border-blue-500 focus:outline-none font-mono"
                  disabled={status === "connecting"}
                  placeholder="season-1"
                />
                <div className="text-xs text-neutral-500">
                  Optional grouping key for VOD playlists (used by profile and upcoming playlist surfaces).
                </div>
                {vodAccessScopeInvalid && (
                  <div className="text-xs text-red-300">Playlist unlock scope requires a Playlist ID.</div>
                )}
              </div>
