...
import { getXmrRefundMinServedBytes, getXmrWalletRpcClient } from "@/lib/monero/server";
...
export async function POST(req: NextRequest, ctx: { params: Promise<{ token: string }> }): Promise<Response> {
  ...
  for (const raw of receipts) {
    ...
    if (parsed.fromPubkey !== session.viewerPubkey) return new Response("receipt does not match session viewer", { status: 400 });
    servedBytes += parsed.servedBytes;
  }

  if (servedBytes < minServedBytes) {
    return new Response(`refund threshold not met (servedBytes=${servedBytes}, required=${minServedBytes})`, { status: 403 });
  }
