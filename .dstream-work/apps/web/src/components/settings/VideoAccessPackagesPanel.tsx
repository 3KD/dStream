...
import {
  ...
  type VideoPackagePurchaseStats,
  type VideoCatalogListRow,
  type VideoAccessPackageStatus,
  type VideoAccessPackageVisibility,
...
import {
  ...
  type VideoReleaseEntitlementCoverage,
  type VideoReleaseEntryInput
} from "@/lib/videoRelease";
import {
  DEFAULT_Video_PURCHASE_POLICY,
  Video_PURCHASE_POLICIES,
  getVideoPurchasePolicyFromMetadata,
...
export function VideoAccessPackagesPanel() {
  ...
  const [visibilityInput, setVisibilityInput] = useState<VideoAccessPackageVisibility>("public");
  ...
  const [purchaseStatsByPackageId, setPurchaseStatsByPackageId] = useState<Record<string, VideoPackagePurchaseStats>>({});
  ...
  const [playlistCatalogRows, setPlaylistCatalogRows] = useState<VideoPlaylistCatalogRow[]>([]);
  ...
  const [catalogRows, setCatalogRows] = useState<VideoCatalogListRow[]>([]);
  ...
  useEffect(() => {
    const onImport = (event: Event) => {
      const detail = (event as CustomEvent<VideoPackageBulkImportDetail>).detail;
      ...
      setBulkNotice(
        ...
        } from Video selection. Review preset and apply.`
      );
    };

    window.addEventListener(Video_PACKAGE_IMPORT_EVENT, onImport as EventListener);
    return () => {
      window.removeEventListener(Video_PACKAGE_IMPORT_EVENT, onImport as EventListener);
    };
  }, []);
  ...
  const catalogReleaseEntries = useMemo<VideoReleaseEntryInput[]>(
    () =>
      catalogRows.map((row) => ({
  ...
  const releaseCoverageByPath = useMemo(
    () => buildVideoPricingCoverage(catalogReleaseEntries, streamScopedPackages),
    [catalogReleaseEntries, streamScopedPackages]
  );
  const releaseSummary = useMemo(
    () => summarizeVideoRelease(catalogReleaseEntries, releaseCoverageByPath),
    [catalogReleaseEntries, releaseCoverageByPath]
  );
  const entitlementCoverageByPath = useMemo<Record<string, VideoReleaseEntitlementCoverage>>(() => {
    if (!normalizedHostPubkey || !normalizedStreamIdInput) return {};
    if (catalogReleaseEntries.length === 0) return {};
    return buildVideoEntitlementCoverage(catalogReleaseEntries, streamEntitlements, {
  ...
  const buildProof = useCallback(async () => {
    ...
    if (!identity?.pubkey) throw new Error("Connect identity to manage Video packages.");
  ...
  const applyPackageToForm = useCallback((row: VideoAccessPackage) => {
  ...
  const loadPlaylistCatalog = useCallback(async () => {
    ...
    try {
      ...
      if (entitlementsResult.ok) {
        const resourcePrefix = `stream:${normalizedHostPubkey}:${streamId}:video`;
        setStreamEntitlements(
          entitlementsResult.entitlements.filter(
            (row) => row.resourceId.startsWith(resourcePrefix) && (row.actions.includes("watch_video") || row.actions.includes("*"))
          )
        );
        setEntitlementsError(null);
      } else {
  ...
  const preparePlaylistPackageDraft = useCallback(
    (playlistRow: VideoPlaylistCatalogRow) => {
      const streamId = streamIdInput.trim();
      if (!STREAM_ID_RE.test(streamId)) {
  ...
  const applyBulkPreset = useCallback(async () => {
    ...
    try {
      ...
      const existingByPlaylist = new Map<string, VideoAccessPackage>();
      for (const row of packages) {
      ...
      const updatedRows: VideoAccessPackage[] = [];
      ...
      for (const playlistId of selectedBulkPlaylistIds) {
        ...
        try {
          const result = await upsertVideoAccessPackageClient({
            hostPubkey: normalizedHostPubkey,
  ...
  const applyBulkFilePreset = useCallback(async () => {
    ...
    try {
      const proof = await buildProof();
      const titlePrefix = bulkTitlePrefixInput.trim() || streamId;
      const existingByRelativePath = new Map<string, VideoAccessPackage>();
      for (const row of packages) {
        if (row.streamId !== streamId) continue;
  ...
  const disablePackage = useCallback(
    async (row: VideoAccessPackage) => {
      ...
      try {
        const proof = await buildProof();
        const result = await disableVideoAccessPackageClient({
          hostPubkey: normalizedHostPubkey,
          packageId: row.id,
          operatorProofEvent: proof
        });
  ...
  const duplicatePackageToDraft = useCallback((row: VideoAccessPackage) => {
    setEditingPackageId(null);
    setStreamIdInput(row.streamId);
    setPlaylistIdInput(row.playlistId ?? "");
    setRelativePathInput(row.relativePath ?? "");
  ...
  const testPurchase = useCallback(
    async (row: VideoAccessPackage) => {
      ...
      try {
        ...
        const result = await purchaseVideoAccessPackageClient({
          packageId: row.id,
      ...
      try {
        ...
        const result = await grantAccessEntitlementClient({
          ...
          metadata: {
            ...
            issuedFrom: "video_packages_panel"
          },
          operatorProofEvent
        });
  ...
  if (!identity) {
    return (
      <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-6 space-y-2">
        <h2 className="text-sm font-semibold text-neutral-200">Video Access Packages</h2>
        <p className="text-sm text-neutral-500">Connect an identity to publish and manage paid Video packages.</p>
      </section>
    );
  }
    ...
    <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-6 space-y-4">
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-950/40 p-4 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
          <select
            ...
            onChange={(event) => setPurchasePolicyInput(event.target.value as VideoPurchasePolicy)}
            className="bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-sm"
          >
            {Video_PURCHASE_POLICIES.map((policy) => (
              <option key={policy} value={policy}>
                {getVideoPurchasePolicyLabel(policy)}
              </option>
            ))}
          </select>
          <select
            value={statusInput}
            onChange={(event) => setStatusInput(event.target.value as VideoAccessPackageStatus)}
            className="bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-sm"
          >
          ...
          <select
            ...
            onChange={(event) => setVisibilityInput(event.target.value as VideoAccessPackageVisibility)}
            className="bg-neutral-950 border border-neutral-800 rounded-xl px-3 py-2 text-sm"
          >
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="text-[11px] text-neutral-500">
            Purchase policy: <span className="text-neutral-300">{getVideoPurchasePolicyLabel(purchasePolicyInput)}</span>
          </div>
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-950/50 p-4 space-y-3">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="text-[11px] text-neutral-500">
            ...
            <span className="text-neutral-300">{getVideoPurchasePolicyLabel(purchasePolicyInput)}</span>
          </div>
          <button
            type="button"
      ...
      {visiblePackages.length === 0 ? (
        <div className="text-sm text-neutral-500">
          {packages.length === 0 ? "No Video packages loaded for this host." : "No packages in the current stream filter."}
        </div>
      ) : (
        <div className="space-y-2">
          {visiblePackages.map((row) => {
            const purchasePolicy = getVideoPurchasePolicyFromMetadata(row.metadata);
            const purchasePolicyLabel = getVideoPurchasePolicyLabel(purchasePolicy);
