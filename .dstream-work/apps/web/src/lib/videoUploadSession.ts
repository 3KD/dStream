...
import { isAllowedVideoRecordingExtension, resolveVideoStreamDir } from "./video";
import { type VideoCatalogEntry, type VideoCatalogVisibility, type VideoProcessingState, upsertVideoCatalogEntry } from "./videoCatalog";
...
export interface VideoUploadSessionCleanupResult {
  scanned: number;
  removed: number;
}
...
function getUploadSessionRootDir(): string {
  const raw = (process.env.DSTREAM_Video_UPLOAD_SESSION_DIR ?? "").trim();
  return raw || "/var/lib/dstream/video-upload-sessions";
}

function sanitizeVisibility(input: string | null | undefined): VideoCatalogVisibility {
  const value = (input ?? "").trim().toLowerCase();
  if (value === "private" || value === "unlisted" || value === "public") return value;
  return "public";
}

function sanitizeProcessingState(input: string | null | undefined): VideoProcessingState {
  const value = (input ?? "").trim().toLowerCase();
...
async function readSession(uploadId: string): Promise<VideoUploadSession> {
  const raw = await readFile(sessionMetaPath(uploadId), "utf8");
  const parsed = JSON.parse(raw) as VideoUploadSession;
...
async function writeSession(session: VideoUploadSession): Promise<void> {
...
async function findAvailableRelativePath(streamDir: string, desiredRelativePath: string): Promise<string> {
  ...
  while (await ensureUploadFileDoesNotExist(path.resolve(streamDir, candidate))) {
    ...
    counter += 1;
    if (counter > 1000) throw new Error("Unable to allocate unique Video filename.");
  }
  return candidate;
}
...
export async function cleanupExpiredVideoUploadSessions(limit = 200): Promise<VideoUploadSessionCleanupResult> {
...
export function resolveHostAndOrigin(hostRaw: string, streamIdRaw: string): {
  ...
  if (!originStreamId) throw new Error("Invalid stream id for Video upload.");
  return { hostPubkey, streamId, originStreamId };
}

export async function startVideoUploadSession(input: {
  hostPubkey: string;
  streamId: string;
  originStreamId: string;
  fileName: string;
  ...
  thumbnailUrl?: string;
  tags?: string[];
}): Promise<{ session: VideoUploadSession; chunkSizeBytes: number }> {
  await cleanupExpiredVideoUploadSessions(300).catch(() => undefined);

  const safeFileName = sanitizeFileName(input.fileName);
  const extension = path.extname(safeFileName).toLowerCase();
  if (!isAllowedVideoRecordingExtension(extension)) {
  ...
  const ttlSecRaw = Number.parseInt((process.env.DSTREAM_Video_UPLOAD_SESSION_TTL_SEC ?? "").trim(), 10);
...
export async function appendVideoUploadSessionChunk(input: {
  uploadId: string;
  uploadToken: string;
  offset: number;
  bytes: Uint8Array;
...
export async function completeVideoUploadSession(input: {
  uploadId: string;
  uploadToken: string;
}): Promise<{
  hostPubkey: string;
  ...
  const streamDir = resolveVideoStreamDir(session.originStreamId);
  if (!streamDir) throw new Error("Invalid stream directory for Video upload.");
  ...
  const entry = upsertVideoCatalogEntry({
...
export async function abortVideoUploadSession(input: { uploadId: string; uploadToken: string }): Promise<void> {
  const session = await readSession(input.uploadId);
  if (input.uploadToken !== session.uploadToken) throw new Error("Upload token mismatch.");
  await deleteSessionArtifacts(session.uploadId);
}
