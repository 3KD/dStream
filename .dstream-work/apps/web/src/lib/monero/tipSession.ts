...
export function makeTipLabel(session: { streamPubkey: string; streamId: string; nonce: string }): string {
  return `dstream_tip:${session.streamPubkey}:${session.streamId}:${session.nonce}`;
}

export function signTipSession(payload: TipSessionV1): string {
...
export function verifyTipSession(token: string): TipSessionV1 | null {
  ...
  if (obj.v !== 1 || obj.t !== "xmr_tip_session") return null;
