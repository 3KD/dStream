...
import { getVideoCatalogEntry } from "./videoCatalog";

const DEFAULT_STORE_PATH = "/var/lib/dstream/video-analytics.json";
const MAX_ENTRIES = 100_000;
const MAX_UNIQUE_VIEWERS_PER_ENTRY = 12_000;
const STREAM_ID_RE = /^[a-zA-Z0-9][a-zA-Z0-9_-]{0,127}$/;

interface VideoAnalyticsStoreShape {
  version: number;
  updatedAtSec: number;
  entries: VideoAnalyticsEntry[];
}

interface VideoAnalyticsEntry {
  id: string;
...
let entriesCache: VideoAnalyticsEntry[] = [];
...
function getStorePath(): string {
  const raw = (process.env.DSTREAM_Video_ANALYTICS_STORE_PATH ?? "").trim();
...
function normalizePlaybackMode(input: unknown): "live" | "video" | undefined {
  const value = typeof input === "string" ? input.trim().toLowerCase() : "";
  if (value === "live" || value === "video") return value;
  return undefined;
}
...
function toSummaryRow(entry: VideoAnalyticsEntry): VideoAnalyticsSummaryRow {
  return {
...
function sanitizeStoredEntry(input: unknown): VideoAnalyticsEntry | null {
  if (!input || typeof input !== "object") return null;
  const row = input as Partial<VideoAnalyticsEntry>;
...
function ensureLoaded(): void {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<VideoAnalyticsStoreShape> | null;
    const rows = Array.isArray(parsed?.entries) ? parsed.entries : [];
    entriesCache = rows.map(sanitizeStoredEntry).filter((row): row is VideoAnalyticsEntry => !!row);
  } catch {
    entriesCache = [];
  }
}
...
function persist(): void {
  try {
    ...
    } satisfies VideoAnalyticsStoreShape);
  } catch {
    // ignore persistence errors
  }
}

export function recordVideoAnalyticsHeartbeat(input: {
  originStreamId: string;
  relativePath: string;
  viewerId?: string;
  elapsedSec?: number;
  ...
  const catalogEntry = getVideoCatalogEntry(parsed.originStreamId, relativePath);
...
export function listVideoAnalyticsSummary(input: {
  originStreamId: string;
  playlistId?: string;
  relativePath?: string;
  limit?: number;
