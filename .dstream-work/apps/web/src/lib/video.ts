...
import { listVideoCatalogEntries, type VideoCatalogVisibility, type VideoProcessingState } from "./videoCatalog";

const ALLOWED_RECORDING_EXTENSIONS = new Set([".mp4", ".m4s", ".mkv", ".ts", ".webm", ".mov"]);
const MAX_LIST_FILES = 256;

function normalizeVideoRoot(raw: string | undefined): string {
  const trimmed = (raw ?? "").trim();
  return trimmed || "/recordings";
}

export function getVideoRootDir(): string {
  return normalizeVideoRoot(process.env.DSTREAM_VIDEO_DIR);
}

export function isAllowedVideoRecordingExtension(extension: string | null | undefined): boolean {
...
export function resolveVideoStreamDir(originStreamId: string): string | null {
  if (!isValidOriginStreamId(originStreamId)) return null;
  const root = getVideoRootDir();
...
export function resolveVideoFile(originStreamId: string, filePathSegments: string[]): string | null {
  const streamDir = resolveVideoStreamDir(originStreamId);
...
export function getVideoFileContentType(filePath: string): string {
...
async function walkRecordingFiles(
  ...
  for (const entry of entries) {
    ...
    if (!isAllowedVideoRecordingExtension(ext)) continue;
...
export async function listVideoRecordings(
...
): Promise<{ files: VideoRecordingFile[]; streamDir: string | null }> {
  const streamDir = resolveVideoStreamDir(originStreamId);
  ...
  const files: VideoRecordingFile[] = [];
  ...
  const catalogEntries = listVideoCatalogEntries({
    originStreamId,
    includePrivate: true,
  ...
  const filtered: VideoRecordingFile[] = [];
  ...
  for (const file of filtered) {
    ...
    file.url = `/api/video/file/${encodeURIComponent(originStreamId)}/${parts.join("/")}`;
  }
