...
export function buildPaymentUri(method: StreamPaymentMethod): string | null {
  ...
  switch (asset) {
    ...
    case "btc":
      return withQuery(`bitcoin:${address}`, { label: method.label });
    case "doge":
      return withQuery(`dogecoin:${address}`, { label: method.label });
    case "bch":
      return withQuery(address.startsWith("bitcoincash:") ? address : `bitcoincash:${address}`, { label: method.label });
    case "ada":
