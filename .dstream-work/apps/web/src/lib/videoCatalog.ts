...
interface VideoCatalogStoreShape {
  version: number;
  updatedAtSec: number;
  entries: VideoCatalogEntry[];
}

const DEFAULT_CATALOG_STORE_PATH = "/var/lib/dstream/video-catalog.json";
...
let loaded = false;
let entriesCache: VideoCatalogEntry[] = [];
...
function getStorePath(): string {
  const raw = (process.env.DSTREAM_Video_CATALOG_STORE_PATH ?? "").trim();
...
function parseVisibility(input: unknown): VideoCatalogVisibility {
...
function parseProcessingState(input: unknown): VideoProcessingState {
  const value = typeof input === "string" ? input.trim().toLowerCase() : "";
...
function parseStoredEntry(input: unknown): VideoCatalogEntry | null {
  if (!input || typeof input !== "object") return null;
  const row = input as Partial<VideoCatalogEntry>;
...
function ensureLoaded(): void {
  ...
  try {
    ...
    const parsed = JSON.parse(raw) as Partial<VideoCatalogStoreShape> | null;
    const rows = Array.isArray(parsed?.entries) ? parsed?.entries ?? [] : [];
    entriesCache = rows.map(parseStoredEntry).filter((row): row is VideoCatalogEntry => !!row);
  } catch {
    entriesCache = [];
  }
}
...
function persist(): void {
  const storePath = getStorePath();
  try {
    const payload: VideoCatalogStoreShape = {
      version: 1,
      updatedAtSec: nowSec(),
...
export function listVideoCatalogEntries(input: {
  originStreamId: string;
...
export function listVideoCatalogOriginStreamIds(limit = 1000): string[] {
  ensureLoaded();
...
export function getVideoCatalogEntry(originStreamId: string, relativePath: string): VideoCatalogEntry | null {
  ensureLoaded();
...
export function upsertVideoCatalogEntry(input: {
  originStreamId: string;
  relativePath: string;
  title?: string;
  description?: string;
  ...
  const created: VideoCatalogEntry = {
...
export function deleteVideoCatalogEntry(input: { originStreamId: string; relativePath: string }): boolean {
  ensureLoaded();
  const parsedOrigin = parseOriginStreamId(input.originStreamId);
  const relativePath = normalizeRelativePath(input.relativePath);
