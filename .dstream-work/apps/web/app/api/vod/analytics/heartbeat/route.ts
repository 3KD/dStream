...
import { recordVodAnalyticsHeartbeat } from "@/lib/vodAnalytics";
...
function parsePlaybackMode(value: unknown): "live" | "vod" | undefined {
  if (typeof value !== "string") return undefined;
  const normalized = value.trim().toLowerCase();
  if (normalized === "live" || normalized === "vod") return normalized;
  return undefined;
}
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const row = recordVodAnalyticsHeartbeat({
      originStreamId,
      relativePath,
      viewerId,
    ...
    return Response.json({ ok: false, error: error?.message ?? "Failed to record VOD analytics heartbeat." }, { status: 400 });
  }
}
