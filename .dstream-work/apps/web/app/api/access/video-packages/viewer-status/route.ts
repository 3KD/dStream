...
function includesWatchVideo(actions: string[]): boolean {
  return actions.includes("watch_video");
}
...
export async function POST(req: Request): Promise<Response> {
  ...
  const resourcePrefix = streamId ? `stream:${hostPubkey}:${streamId}:video` : `stream:${hostPubkey}:`;

  const unlocks = rows
    .filter((row) => {
      if (!includesWatchVideo(row.actions)) return false;
      if (streamId) return row.resourceId.startsWith(resourcePrefix);
      return row.resourceId.startsWith(resourcePrefix) && row.resourceId.includes(":video");
    })
    .map((row) => ({
