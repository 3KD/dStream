...
import { processVideoCatalogEntries } from "@/lib/videoProcessing";
...
export async function POST(
  ...
  if (!originStreamId) {
    return Response.json({ ok: false, error: "Invalid stream id for Video processing." }, { status: 400 });
  }
  ...
  try {
    const result = await processVideoCatalogEntries({ originStreamId, limit });
    ...
    return Response.json({ ok: false, error: error?.message ?? "Failed to process Video entries." }, { status: 400 });
  }
}
