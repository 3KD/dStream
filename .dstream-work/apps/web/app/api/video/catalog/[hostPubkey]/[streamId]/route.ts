...
interface VideoPlaylistCatalogRow {
  id: string;
  fileCount: number;
...
): VideoPlaylistCatalogRow[] {
  const playlistMap = new Map<string, VideoPlaylistCatalogRow>();
...
export async function POST(
  ...
  const listResult = await listVideoRecordings(originStreamId);
