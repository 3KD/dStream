import { listVideoAnalyticsSummary } from "@/lib/videoAnalytics";
import { resolveHostAndOrigin } from "@/lib/videoUploadSession";
...
export async function POST(
  ...
  try {
    const rows = listVideoAnalyticsSummary({
    ...
    return Response.json({ ok: false, error: error?.message ?? "Failed to load Video analytics summary." }, { status: 400 });
  }
}
