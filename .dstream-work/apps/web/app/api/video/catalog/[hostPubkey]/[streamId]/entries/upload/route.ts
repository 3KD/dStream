...
import { isAllowedVideoRecordingExtension, resolveVideoStreamDir } from "@/lib/video";
import { upsertVideoCatalogEntry, type VideoCatalogVisibility, type VideoProcessingState } from "@/lib/videoCatalog";
...
function sanitizeVisibility(input: string | null): VideoCatalogVisibility {
  const value = (input ?? "").trim().toLowerCase();
  if (value === "private" || value === "unlisted" || value === "public") return value;
  return "public";
}

function sanitizeProcessingState(input: string | null): VideoProcessingState {
  const value = (input ?? "").trim().toLowerCase();
...
export async function POST(
  ...
  if (!originStreamId) {
    return Response.json({ ok: false, error: "Invalid stream id for Video upload." }, { status: 400 });
  }
  ...
  if (!streamDir) {
    return Response.json({ ok: false, error: "Invalid stream directory for Video upload." }, { status: 400 });
  }
  ...
  try {
    ...
    return Response.json({ ok: false, error: error?.message ?? "Failed to write uploaded Video file." }, { status: 500 });
  }
  ...
  try {
    ...
    return Response.json({ ok: false, error: error?.message ?? "Failed to upsert Video metadata." }, { status: 400 });
  }
}
