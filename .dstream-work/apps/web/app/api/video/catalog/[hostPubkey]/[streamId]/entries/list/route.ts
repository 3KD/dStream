...
import { listVideoRecordings } from "@/lib/video";
import { listVideoCatalogEntries, type VideoCatalogEntry } from "@/lib/videoCatalog";
import { authorizeAccessAdmin, asString, parseBoolean } from "../../../../../../access/_lib";

export const runtime = "nodejs";
export const dynamic = "force-dynamic";

interface VideoCatalogListRow {
  relativePath: string;
  fileName: string;
...
export async function POST(
  ...
  const { files } = await listVideoRecordings(originStreamId, {
  ...
  const metadataByPath = new Map<string, VideoCatalogEntry>();
  for (const entry of listVideoCatalogEntries({
    originStreamId,
  ...
  const rows: VideoCatalogListRow[] = files.map((file) => ({
