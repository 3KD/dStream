...
import { processVideoCatalogEntriesForHost } from "@/lib/videoProcessing";
...
export async function POST(
  ...
  try {
    const result = await processVideoCatalogEntriesForHost({
      hostPubkey,
      limit,
      maxStreams
    });
    ...
    return Response.json({ ok: false, error: error?.message ?? "Failed to process Video entries for host." }, { status: 400 });
  }
}
