...
import { buildStreamAnnounceEvent, makeStreamKey, type StreamAnnounce, type StreamVideoPolicy } from "@dstream/protocol";
...
type VideoModePatch = "keep" | "public" | "paid";
...
interface LocalVideoEntitlement {
  key: string;
...
interface VideoPatchPresetValues {
...
interface VideoPatchPreset {
  id: string;
  name: string;
  createdAt: number;
  updatedAt: number;
  values: VideoPatchPresetValues;
}

const STREAM_ENTITLEMENT_PREFIX = "dstream_video_access_v1:";
const PLAYLIST_ENTITLEMENT_PREFIX = "dstream_video_playlist_access_v1:";
const Video_PATCH_PRESETS_STORAGE_KEY = "dstream_video_patch_presets_v1";
...
function readVideoPatchPresetsFromStorage(): VideoPatchPreset[] {
  if (typeof window === "undefined") return [];
  const raw = localStorage.getItem(Video_PATCH_PRESETS_STORAGE_KEY);
  if (!raw) return [];
  ...
  const presets: VideoPatchPreset[] = [];
  for (const item of parsed) {
    ...
    const videoModePatch: VideoModePatch =
      values.videoModePatch === "public" || values.videoModePatch === "paid" ? values.videoModePatch : "keep";
...
function writeVideoPatchPresetsToStorage(presets: VideoPatchPreset[]): void {
  if (typeof window === "undefined") return;
  localStorage.setItem(Video_PATCH_PRESETS_STORAGE_KEY, JSON.stringify(presets));
}

function readVideoEntitlementsFromStorage(nowMs = Date.now()): LocalVideoEntitlement[] {
  if (typeof window === "undefined") return [];

  const entries: LocalVideoEntitlement[] = [];
...
export default function VideoPage() {
  ...
  const [mode, setMode] = useState<VideoModeFilter>("all");
  const [favoritesOnly, setFavoritesOnly] = useState(false);
  const [creatorParam, setCreatorParam] = useState("");
  const [videoOverrides, setVideoOverrides] = useState<Record<string, StreamVideoPolicy>>({});
  ...
  const [videoModePatch, setVideoModePatch] = useState<VideoModePatch>("keep");
  ...
  const [entitlements, setEntitlements] = useState<LocalVideoEntitlement[]>([]);
  ...
  const [savedPresets, setSavedPresets] = useState<VideoPatchPreset[]>([]);
  ...
  useEffect(() => {
    const presets = readVideoPatchPresetsFromStorage();
  ...
  const streamsWithOverrides = useMemo(() => {
    if (Object.keys(videoOverrides).length === 0) return streams;
    return streams.map((stream) => {
      const key = makeStreamKey(stream.pubkey, stream.streamId);
      const override = videoOverrides[key];
      return override ? { ...stream, video: override } : stream;
    });
  }, [streams, videoOverrides]);
  ...
  const myVideoStreams = useMemo(() => {
    if (!identity) return [];
    return videoStreams.filter((stream) => stream.pubkey === identity.pubkey);
  }, [identity, videoStreams]);

  const sortedMyVideoStreams = useMemo(() => {
    const list = [...myVideoStreams];
    list.sort((a, b) => {
      ...
      const aPlaylist = resolveVideoPolicy(a).playlistId?.trim() || "Ungrouped";
      const bPlaylist = resolveVideoPolicy(b).playlistId?.trim() || "Ungrouped";
    ...
    return list;
  }, [creatorListSortMode, myVideoStreams]);

  useEffect(() => {
    setSelectedStreamKeys((prev) => {
      const allowed = new Set(sortedMyVideoStreams.map((stream) => makeStreamKey(stream.pubkey, stream.streamId)));
  ...
  const currentPresetValues = useMemo<VideoPatchPresetValues>(
    () => ({
    ...
    [accessHoursPatchInput, accessPatchMode, playlistScopePatch, playlistTargetInput, pricePatchMode, priceXmrPatchInput, videoModePatch]
  );
  ...
  const saveCurrentPreset = useCallback(() => {
    ...
    let next: VideoPatchPreset[];
    let nextId = selectedPresetId;
    if (existing) {
      const updated: VideoPatchPreset = {
        ...existing,
        name: presetName,
        updatedAt: now,
      ...
      nextId = id;
      const created: VideoPatchPreset = {
        id,
        name: presetName,
        createdAt: now,
        updatedAt: now,
    ...
    writeVideoPatchPresetsToStorage(next);
  ...
  const loadSelectedPreset = useCallback(() => {
    ...
    setVideoModePatch(preset.values.videoModePatch);
  ...
  const applyPlaylistQuickFilter = useCallback((playlistId: string, filterMode: VideoModeFilter = "all") => {
  ...
  const allMineSelected = useMemo(() => {
    if (sortedMyVideoStreams.length === 0) return false;
    return sortedMyVideoStreams.every((stream) => selectedStreamKeys[makeStreamKey(stream.pubkey, stream.streamId)]);
  }, [sortedMyVideoStreams, selectedStreamKeys]);
  ...
  const toggleSelectAllMine = useCallback(() => {
    if (sortedMyVideoStreams.length === 0) return;
    if (allMineSelected) {
      setSelectedStreamKeys({});
      return;
    }
    const next: Record<string, boolean> = {};
    for (const stream of sortedMyVideoStreams) {
      next[makeStreamKey(stream.pubkey, stream.streamId)] = true;
    }
    setSelectedStreamKeys(next);
  }, [allMineSelected, sortedMyVideoStreams]);

  const executeVideoPatch = useCallback(
    async (
      targetStreams: StreamAnnounce[],
      options?: { targetPlaylistId?: string; clearSelectionOnSuccess?: boolean }
    ) => {
      if (!identity) {
        setPlaylistActionError("Connect an identity first to republish your Video metadata.");
        return;
      }
      ...
      const wouldTouchPaid =
        videoModePatch === "paid" ||
        (videoModePatch === "keep" && targetStreams.some((stream) => resolveVideoPolicy(stream).mode === "paid"));
      ...
      if (videoModePatch === "paid" && pricePatchMode !== "set") {
      ...
      for (const stream of targetStreams) {
        try {
          const nextVideo = buildUpdatedVideoPolicy(
            stream,
            targetPlaylistId,
            playlistScopePatch,
            videoModePatch,
          ...
          setVideoOverrides((prev) => ({
            ...prev,
            [makeStreamKey(stream.pubkey, stream.streamId)]: nextVideo
          }));
          success += 1;
        } catch (error: any) {
  ...
  const applyPlaylistPatch = useCallback(async () => {
    const targetStreams = sortedMyVideoStreams.filter((stream) => selectedStreamKeys[makeStreamKey(stream.pubkey, stream.streamId)]);
    await executeVideoPatch(targetStreams, { clearSelectionOnSuccess: true });
  }, [executeVideoPatch, selectedStreamKeys, sortedMyVideoStreams]);
  ...
  const playlistSummaries = useMemo(() => {
    ...
    return list;
  }, [identity, playlistSortMode, videoStreams]);

  const selectMineForPlaylist = useCallback(
    (playlistId: string, mode: "only" | "add" = "only") => {
      if (!identity) return;
      const matching = sortedMyVideoStreams.filter((stream) => {
        const value = resolveVideoPolicy(stream).playlistId?.trim() || "Ungrouped";
  ...
  const applyPresetToPlaylist = useCallback(
    async (playlistId: string) => {
      ...
      await executeVideoPatch(matching, {
        targetPlaylistId: currentTarget || fallbackPlaylistId
      });
    },
    [executeVideoPatch, identity, playlistTargetInput, sortedMyVideoStreams]
  );

  const refreshEntitlements = useCallback(() => {
    setEntitlements(readVideoEntitlementsFromStorage());
  }, []);
      ...
      <main className="max-w-[1800px] mx-auto p-8 space-y-8">
        <header className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="text-sm text-neutral-400">{videoStreams.length} replay items</div>
          </div>
          {creatorFilter && (
            <div className="text-xs text-neutral-500">
              ...
              ·{" "}
              <Link href="/video" className="text-blue-300 hover:text-blue-200">
                clear
              </Link>
            </div>
          )}
          <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_auto_auto_auto] gap-3 items-center">
            ...
            <select
              value={mode}
              onChange={(e) => setMode(e.target.value as VideoModeFilter)}
        ...
        <section className="space-y-3 rounded-2xl border border-neutral-800 bg-neutral-900/40 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <div className="text-xs text-neutral-500">{myVideoStreams.length} replay item{myVideoStreams.length === 1 ? "" : "s"} owned by your current identity</div>
          </div>
          {!identity ? (
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_220px_auto] gap-3 items-end">
                ...
                <label className="space-y-1">
                  <div className="text-xs text-neutral-400">Paid Video scope patch</div>
                  <select
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[200px_180px_minmax(0,1fr)_180px_minmax(0,1fr)] gap-3 items-end">
                <label className="space-y-1">
                  ...
                  <select
                    value={videoModePatch}
                    onChange={(e) => setVideoModePatch(e.target.value as VideoModePatch)}
                    className="w-full bg-neutral-900 border border-neutral-800 rounded-xl px-3 py-2 text-sm text-neutral-200 focus:border-blue-500 focus:outline-none"
                  >
              ...
              <div className="max-h-72 overflow-auto rounded-xl border border-neutral-800 bg-neutral-950/40 divide-y divide-neutral-800">
                {sortedMyVideoStreams.map((stream) => {
                  const streamKey = makeStreamKey(stream.pubkey, stream.streamId);
                  const policy = resolveVideoPolicy(stream);
                    ...
                    <label key={`manage:${streamKey}`} className="flex items-start gap-3 px-3 py-2 cursor-pointer">
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="text-[11px] text-neutral-500">
                          {videoModeLabel(policy)} · playlist {(policy.playlistId ?? "Ungrouped")} · {new Date(stream.createdAt * 1000).toLocaleString()}
                        </div>
                      </div>
                    </label>
                  );
        ...
        <section className="space-y-3 rounded-2xl border border-neutral-800 bg-neutral-900/40 p-4">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <h2 className="text-lg font-semibold">Local Video Access Entitlements</h2>
            <button
              type="button"
