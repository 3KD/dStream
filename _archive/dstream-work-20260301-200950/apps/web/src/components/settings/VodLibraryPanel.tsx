...
import {
  ...
  ingestVodCatalogEntriesClient,
  listVodAccessPackagesClient,
  listVodAnalyticsSummaryClient,
  listVodCatalogEntriesClient,
  processVodCatalogHostEntriesClient,
  processVodCatalogEntriesClient,
  uploadVodCatalogFileClient,
  upsertVodCatalogEntryClient,
  type VodAnalyticsSummaryRow,
...
type VodListSort = "modified_desc" | "modified_asc" | "title_asc";
...
function draftFromRow(row: VodCatalogListRow): RowDraft {
...
export function VodLibraryPanel() {
  ...
  const [ingestVisibility, setIngestVisibility] = useState<VodCatalogVisibility>("public");
  const [ingestProcessingState, setIngestProcessingState] = useState<VodProcessingState>("ready");
  ...
  const [analyticsByPath, setAnalyticsByPath] = useState<Record<string, VodAnalyticsSummaryRow>>({});
  ...
  const [sortMode, setSortMode] = useState<VodListSort>("modified_desc");
  ...
  const [bulkVisibilityInput, setBulkVisibilityInput] = useState<VodCatalogVisibility>("public");
  ...
  const releaseRows = useMemo<VodReleaseEntryInput[]>(
    () =>
      rows.map((row) => {
        const draft = drafts[row.relativePath] ?? draftFromRow(row);
        return {
  ...
  const releaseCoverageByPath = useMemo(() => buildVodPricingCoverage(releaseRows, packages), [packages, releaseRows]);
  const releaseSummary = useMemo(() => summarizeVodRelease(releaseRows, releaseCoverageByPath), [releaseCoverageByPath, releaseRows]);
  ...
  const buildProof = useCallback(async () => {
    ...
    if (!identity?.pubkey) throw new Error("Connect identity to curate VOD entries.");
    const proof = await buildAccessAdminProof(signEvent, identity.pubkey, normalizedHost);
    if (!proof) throw new Error("Failed to sign VOD admin proof.");
    return proof;
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      const [result, analyticsResult, packageResult] = await Promise.all([
        ...
        listVodAccessPackagesClient({
          ...
          .catch((packageError: any) => ({
            ok: false as const,
            error: packageError?.message ?? "Failed to load VOD access packages."
          }))
      ]);
      setRows(result.rows);
      setDrafts(() => {
      ...
      setNotice(
        `Loaded ${result.count} VOD recording${result.count === 1 ? "" : "s"} and ${
      ...
      setPackages([]);
      setError(loadError?.message ?? "Failed to load VOD library.");
    } finally {
      setIsLoading(false);
  ...
  const importRowsToPackageBulk = useCallback(
    (
      ...
      const detail: VodPackageBulkImportDetail = {
        hostPubkey: normalizedHost,
        streamId: normalizedStreamId,
        playlistIds,
        relativePaths,
      ...
      window.dispatchEvent(new CustomEvent(VOD_PACKAGE_IMPORT_EVENT, { detail }));
      setError(null);
      const scopeSummary =
      ...
      setNotice(`${options.noticePrefix} Queued ${scopeSummary} in VOD package pricing.`);
      return true;
    },
  ...
  const setPublishedForSelection = useCallback((published: boolean) => {
    if (selectedRelativePaths.length === 0) {
      setError("Select at least one VOD row.");
      return;
    }
    setError(null);
    setNotice(null);
  ...
  const setVisibilityForSelection = useCallback((visibility: VodCatalogVisibility) => {
    if (selectedRelativePaths.length === 0) {
      setError("Select at least one VOD row.");
      return;
    }
    ...
    try {
      ...
      for (const relativePath of selectedRelativePaths) {
        ...
        try {
          ...
          const result = await upsertVodCatalogEntryClient({
            hostPubkey: normalizedHost,
            streamId: normalizedStreamId,
            relativePath: row.relativePath,
            operatorProofEvent: proof,
      ...
      if (failed > 0) {
        ...
        setNotice(`Saved ${success} VOD row${success === 1 ? "" : "s"}.`);
      }
    } catch (bulkError: any) {
      setError(bulkError?.message ?? "Failed to save selected VOD rows.");
    } finally {
  ...
  const clearMetadataForSelection = useCallback(async () => {
    ...
    const confirmed = window.confirm(
      `Clear metadata for ${selectedRelativePaths.length} selected VOD row${selectedRelativePaths.length === 1 ? "" : "s"}? This does not delete media files.`
    );
    if (!confirmed) return;
    ...
    try {
      ...
      for (const relativePath of selectedRelativePaths) {
        ...
        try {
          await deleteVodCatalogEntryClient({
            hostPubkey: normalizedHost,
            streamId: normalizedStreamId,
            relativePath: row.relativePath,
            operatorProofEvent: proof
  ...
  const importSelectionToPackageBulk = useCallback(() => {
    ...
    setError(null);
    setNotice(
      `Sent ${playlistIds.length} playlist scope${playlistIds.length === 1 ? "" : "s"} to VOD package pricing.`
    );
  ...
  const importSelectionFilesToPackageBulk = useCallback(() => {
    ...
    window.dispatchEvent(new CustomEvent(VOD_PACKAGE_IMPORT_EVENT, { detail }));
    setError(null);
    setNotice(`Sent ${relativePaths.length} file scope${relativePaths.length === 1 ? "" : "s"} to VOD package pricing.`);
  }, [normalizedHost, normalizedStreamId, rows, selectedRelativePathSet, selectedRelativePaths.length]);

  const saveRow = useCallback(
    async (row: VodCatalogListRow) => {
      setError(null);
      setNotice(null);
      if (!normalizedHost) {
        setError("Host pubkey must be a valid npub or 64-hex pubkey.");
      ...
      try {
        ...
        setError(saveError?.message ?? "Failed to save VOD entry.");
      } finally {
  ...
  const ingestRows = useCallback(async () => {
    ...
    try {
      const proof = await buildProof();
      const result = await ingestVodCatalogEntriesClient({
        hostPubkey: normalizedHost,
        streamId: normalizedStreamId,
        operatorProofEvent: proof,
      ...
      setError(ingestError?.message ?? "Failed to ingest VOD files.");
    } finally {
      setIngestBusy(false);
    }
  }, [
  ...
  const refreshAnalytics = useCallback(async () => {
    ...
    try {
      const proof = await buildProof();
      const result = await listVodAnalyticsSummaryClient({
        hostPubkey: normalizedHost,
        streamId: normalizedStreamId,
        operatorProofEvent: proof,
        limit: 2000
      ...
      setNotice(`Loaded analytics for ${result.count} VOD entr${result.count === 1 ? "y" : "ies"}.`);
    } catch (analyticsError: any) {
      setError(analyticsError?.message ?? "Failed to refresh VOD analytics.");
    } finally {
      setAnalyticsLoading(false);
    }
  }, [buildProof, normalizedHost, normalizedStreamId]);
  ...
  const processRows = useCallback(async () => {
    ...
    try {
      const proof = await buildProof();
      const result = await processVodCatalogEntriesClient({
        hostPubkey: normalizedHost,
        streamId: normalizedStreamId,
        operatorProofEvent: proof,
        limit: 500
      ...
      setError(processError?.message ?? "Failed to process queued VOD entries.");
    } finally {
  ...
  const processHostRows = useCallback(async () => {
    ...
    try {
      const proof = await buildProof();
      const result = await processVodCatalogHostEntriesClient({
        hostPubkey: normalizedHost,
        operatorProofEvent: proof,
        limit: 2000,
        maxStreams: 300
      ...
      setError(processError?.message ?? "Failed to process host VOD queue.");
    } finally {
      setProcessBusy(false);
    }
  ...
  const uploadSelectedFile = useCallback(async () => {
    ...
    if (!uploadFile) {
      setError("Select a VOD file to upload.");
      return;
    }
    ...
    try {
      const proof = await buildProof();
      const result = await uploadVodCatalogFileClient({
        hostPubkey: normalizedHost,
        streamId: normalizedStreamId,
        operatorProofEvent: proof,
        file: uploadFile,
      ...
      setError(uploadError?.message ?? "Failed to upload VOD file.");
    } finally {
  ...
  return (
    <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-6 space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-neutral-200">VOD Library Curation</h2>
          <div className="text-xs text-neutral-500">
            Publish metadata, playlist grouping, visibility, and release state for recorded VOD files.
          </div>
        </div>
      ...
      <div className="rounded-xl border border-neutral-800 bg-neutral-950/40 p-3 space-y-2">
        <div className="text-xs font-semibold text-neutral-200">Upload New VOD File</div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-neutral-800 bg-neutral-950/40 px-3 py-2">
        <label className="inline-flex items-center gap-2 text-xs text-neutral-300">
          ...
          <select
            ...
            onChange={(event) => setIngestVisibility(event.target.value as VodCatalogVisibility)}
        ...
        <label className="inline-flex items-center gap-2 text-xs text-neutral-300">
          ...
          <select
            ...
            onChange={(event) => setIngestProcessingState(event.target.value as VodProcessingState)}
            className="bg-neutral-900 border border-neutral-700 rounded-lg px-2 py-1 text-xs"
          >
      ...
      {rows.length === 0 ? (
        <div className="rounded-xl border border-neutral-800 bg-neutral-950/40 px-4 py-3 text-xs text-neutral-500">
          No VOD files loaded yet. Select host + stream and press <span className="text-neutral-300">Refresh Files</span>.
        </div>
      ) : (
        <div className="space-y-3">
          <div className="rounded-xl border border-neutral-800 bg-neutral-950/40 p-3 space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-2">
              ...
              <select
                value={sortMode}
                onChange={(event) => setSortMode(event.target.value as VodListSort)}
                className="bg-neutral-950 border border-neutral-800 rounded-lg px-2.5 py-1.5 text-sm"
              >
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <select
                  ...
                  onChange={(event) => setBulkVisibilityInput(event.target.value as VodCatalogVisibility)}
              ...
              <div key={row.relativePath} className="rounded-xl border border-neutral-800 bg-neutral-950/40 p-3 space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  <select
                    ...
                    onChange={(event) => updateDraft(row.relativePath, { visibility: event.target.value as VodCatalogVisibility })}
                    className="bg-neutral-950 border border-neutral-800 rounded-lg px-2.5 py-1.5 text-sm"
                  >
                    <option value="public">public</option>
                    <option value="unlisted">unlisted</option>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateDraft(row.relativePath, {
                        processingState: event.target.value as VodProcessingState
                      })
                    }
                    className="bg-neutral-950 border border-neutral-800 rounded-lg px-2.5 py-1.5 text-sm"
                  >
