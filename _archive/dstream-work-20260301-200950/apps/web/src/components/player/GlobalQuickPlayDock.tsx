...
export function GlobalQuickPlayDock() {
  ...
  const isWatchRoute = useMemo(() => {
    if (!hasResolvedPathname) return false;
    return /(?:^|\/)watch\/[^/]+\/[^/]+(?:\/|$)/.test(pathname);
  }, [hasResolvedPathname, pathname]);
  ...
  const mobileChromeHideTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const watchHref = quickPlayStream
    ...
    : null;
  const bottomBarInsetClass = touchDevice ? "mx-1" : "mx-8";
  ...
  const canJumpToLive = hasSeekWindow && liveLagSeconds > 1.5;
  const isAtLiveEdge = !hasSeekWindow || liveLagSeconds <= 1.5;
  ...
  }, [hlsSrc, identity?.pubkey, quickPlayStream]);
  ...
  const clearMobileChromeHideTimer = useCallback(() => {
    if (!mobileChromeHideTimerRef.current) return;
    clearTimeout(mobileChromeHideTimerRef.current);
  ...
  const revealMobileChrome = useCallback(
    (hideAfterMs = 2400) => {
      ...
      mobileChromeHideTimerRef.current = setTimeout(() => {
        setMobileChromeVisible(false);
      }, hideAfterMs);
    },
    [clearMobileChromeHideTimer, touchDevice]
  );
  ...
  useEffect(() => {
    ...
    setMobileChromeVisible(false);
  }, [clearMobileChromeHideTimer, touchDevice, quickPlayStream?.streamId, quickPlayStream?.streamPubkey]);
    ...
    const attachVideo = () => {
      ...
      const syncTimeline = () => {
        try {
          let start = 0;
          let end = 0;
          const seekable = found.seekable;
          if (seekable && seekable.length > 0) {
            start = seekable.start(0);
            end = seekable.end(seekable.length - 1);
          } else if (Number.isFinite(found.duration) && found.duration > 0) {
            end = found.duration;
          }
          const currentTime = Number.isFinite(found.currentTime) ? found.currentTime : 0;
      ...
      found.addEventListener("loadedmetadata", syncTimeline);
      found.addEventListener("durationchange", syncTimeline);
      found.addEventListener("timeupdate", syncTimeline);
      found.addEventListener("progress", syncTimeline);
      found.addEventListener("seeking", syncTimeline);
      found.addEventListener("seeked", syncTimeline);
      syncTimeline();
      ...
      detachListeners = () => {
        ...
        found.removeEventListener("enterpictureinpicture", onEnterPip as any);
        found.removeEventListener("leavepictureinpicture", onLeavePip as any);
        found.removeEventListener("webkitpresentationmodechanged", onWebkitPip as any);
        found.removeEventListener("loadedmetadata", syncTimeline);
        found.removeEventListener("durationchange", syncTimeline);
        found.removeEventListener("timeupdate", syncTimeline);
        found.removeEventListener("progress", syncTimeline);
        found.removeEventListener("seeking", syncTimeline);
        found.removeEventListener("seeked", syncTimeline);
      };
    };
  ...
  useEffect(() => {
    return () => clearMobileChromeHideTimer();
  }, [clearMobileChromeHideTimer]);

  useEffect(() => {
    if (!ready || !hasResolvedPathname || isWatchRoute || !quickPlayStream || !hlsSrc || touchDevice) {
      clearAutoPipTimer();
      return;
    }
    ...
    return () => clearAutoPipTimer();
  }, [clearAutoPipTimer, hasResolvedPathname, hlsSrc, isWatchRoute, pathname, quickPlayStream, ready, requestSystemPip, touchDevice]);
  ...
  }, [requestSystemPip, revealMobileChrome]);
  ...
  }, [hasSeekWindow, revealMobileChrome, timelineEnd, timelineStart]);

  if (!ready || !hasResolvedPathname || isWatchRoute || !quickPlayStream || !hlsSrc) return null;

  return (
    <div
      onMouseDown={handleContainerMouseDown}
      onTouchStartCapture={() => {
        if (touchDevice) revealMobileChrome();
      }}
    ...
    >
      ...
      {touchDevice && !mobileChromeVisible ? (
        <button
          type="button"
          onClick={(event) => {
            event.stopPropagation();
          ...
          className="absolute inset-0 z-30 flex items-center justify-center bg-transparent pointer-events-auto"
          aria-label="Show mini player controls"
          title="Show controls"
        >
          <span className="rounded-lg border border-white/25 bg-black/65 px-3 py-1 text-[11px] font-medium text-white/90">
            Tap for controls
          </span>
        </button>
      ) : null}
      ...
      <div
        className={`absolute inset-0 z-20 flex flex-col pointer-events-none transition-opacity duration-150 ${
          touchDevice ? (mobileChromeVisible ? "opacity-100" : "opacity-0") : "opacity-100"
        }`}
      >
        <div
          className={`p-3 bg-gradient-to-b from-black/80 to-transparent flex items-center gap-3 ${
            touchDevice && !mobileChromeVisible ? "pointer-events-none" : "pointer-events-auto"
          }`}
          ...
          <div className="flex-1 min-w-0">
            ...
            {watchHref ? (
              <Link
                ...
                className="block text-xs font-medium text-white/70 hover:text-white truncate leading-none transition-colors"
                title={quickPlayStream.title || "Live stream"}
              >
        ...
        <div className="mt-auto px-2 pb-2 pt-1.5 bg-gradient-to-t from-black/80 to-transparent pointer-events-none">
          <div
            className={`flex items-center gap-2 ${touchDevice && !mobileChromeVisible ? "pointer-events-none" : "pointer-events-auto"} ${bottomBarInsetClass}`}
            onPointerDownCapture={() => {
              if (touchDevice) revealMobileChrome();
            }}
            onTouchStartCapture={() => {
          ...
          >
              ...
              <button
                ...
                className={`rounded-lg border px-2 py-1 text-[11px] ${
                  isAtLiveEdge
                    ...
                    : "border-white/20 bg-black/40 text-white/90 hover:text-white disabled:opacity-60"
                }`}
            ...
            <input
              ...
              className="min-w-0 flex-1 max-w-[11rem] accent-blue-500"
