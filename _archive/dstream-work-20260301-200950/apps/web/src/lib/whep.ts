...
export class WhepClient {
  ...
  async start(opts?: { timeoutMs?: number }): Promise<WhepStartResult> {
    ...
    // If negotiation fails before we await `trackPromise` (e.g. POST/PATCH/session errors),
    // keep the timer rejection from surfacing as an unhandled promise rejection in the browser.
    void trackPromise.catch(() => undefined);
