...
function normalizeFeaturedStreamId(value: string): string {
  ...
  const origin = decoded.match(ORIGIN_STREAM_ID_PATTERN);
  return (origin?.[1]?.trim() || decoded).replace(TRANSCODE_VARIANT_PATTERN, "").toLowerCase();
}

function featuredCanonicalKey(row: FeaturedRow): string {
  const fromAnnounce = normalizeFeaturedStreamId(row.announce?.streamId ?? "");
  if (fromAnnounce) return fromAnnounce;
  const fromRef = normalizeFeaturedStreamId(row.ref.streamId);
  if (fromRef) return fromRef;
  return `${row.ref.streamPubkey.toLowerCase()}:${row.ref.streamId.trim().toLowerCase()}`;
}

function featuredRowScore(row: FeaturedRow): number {
  const announce = row.announce;
  if (!announce) return -100;
  let score = 0;
  const streaming = (announce.streaming ?? "").toLowerCase();
  if (announce.status === "live") score += 20;
  if (streaming.includes("dstream.stream/stream/")) score += 8;
  else if (streaming.includes("/stream/")) score += 2;
  if (streaming.includes("/index.m3u8") || streaming.endsWith(".m3u8")) score += 3;
  if (announce.image?.trim()) score += 4;
  if (streaming.includes("trycloudflare.com") || streaming.includes("host.docker.internal") || streaming.includes("localhost")) {
    score -= 10;
  }
  return score;
}

function pickFeaturedRow(current: FeaturedRow, candidate: FeaturedRow): FeaturedRow {
  const currentScore = featuredRowScore(current);
  const candidateScore = featuredRowScore(candidate);
  if (candidateScore !== currentScore) return candidateScore > currentScore ? candidate : current;
  const currentCreatedAt = current.announce?.createdAt ?? 0;
  const candidateCreatedAt = candidate.announce?.createdAt ?? 0;
  if (candidateCreatedAt !== currentCreatedAt) return candidateCreatedAt > currentCreatedAt ? candidate : current;
  return current;
}
...
export default function GuildDetailPage() {
  ...
  const featuredRows = useMemo(() => {
    const rows: FeaturedRow[] = visibleRefs.map((ref) => {
      const key = makeStreamKey(ref.streamPubkey, ref.streamId);
      const announce = streamsByKey[key];
      return { ref, key, announce };
    });
    const deduped = new Map<string, FeaturedRow>();
    for (const row of rows) {
      const key = featuredCanonicalKey(row);
      const existing = deduped.get(key);
      deduped.set(key, existing ? pickFeaturedRow(existing, row) : row);
    }
