...
function createSyntheticStream(opts: { width?: number; height?: number; fps?: number; label?: string; onLog?: (line: string) => void }) {
  ...
  const stream: MediaStream = captureStream(0);
  ...
  if (!requestFrame) {
    ...
    const fallbackStream: MediaStream = captureStream(fps);
    const fallbackVideoTrack: any = fallbackStream.getVideoTracks()[0];
    const fallbackRequestFrame =
      typeof fallbackVideoTrack?.requestFrame === "function" ? () => fallbackVideoTrack.requestFrame() : null;
    return createSyntheticStreamWithSource({
      canvas,
      ctx,
      width,
      height,
...
function createSyntheticStreamWithSource(opts: {
  ...
  const { canvas, ctx, width, height, fps, stream, requestFrame } = opts;
  ...
  opts.onLog?.(
    `Synthetic stream created (${width}x${height}@${fps}). Tracks: ${stream
      .getTracks()
      .map((t) => t.kind)
      .join(", ")}${requestFrame ? " (requestFrame)" : ""}`
  );
...
export default function E2EClient() {
  ...
  const videoOnly = useMemo(() => {
    try {
      return new URLSearchParams(window.location.search).get("videoOnly") === "1";
    } catch {
      return false;
    }
  }, []);
  const noHlsOverride = useMemo(() => {
    try {
      return new URLSearchParams(window.location.search).get("noHlsOverride") === "1";
    } catch {
      return false;
    }
  }, []);
  ...
  }, [chatRxStep, expectedChatId, observedChatMessages, pushLog]);

  useEffect(() => {
    if (videoOnly) return;
    if (chatRxStep !== "running") return;
    const timer = setTimeout(() => setFatal("Chat receive timeout."), 20_000);
    return () => clearTimeout(timer);
  }, [chatRxStep, setFatal, videoOnly]);
  ...
  const announce = useCallback(
    async (nextStatus: "live" | "ended") => {
      ...
      for (let attempt = 0; attempt < 5; attempt++) {
        ok = await publishEvent(relays, signed);
        if (ok) break;
        await sleep(500);
      }
  ...
  const run = useCallback(async () => {
    ...
    setChatStep(videoOnly ? "pending" : "running");
    setChatRxStep("pending");
    setPresenceStep(videoOnly ? "pending" : "running");
    setP2pStep(videoOnly ? "pending" : "running");
    ...
    if (videoOnly) pushLog("Mode: video-only checks enabled");
    if (noHlsOverride) pushLog("Mode: watch HLS override disabled");
    ...
    try {
      ...
      setWhipStep("fail");
      setFatal(e?.message ?? "WHIP publish failed.");
      return;
    }
	    ...
	    try {
	      let ok = false;
	      for (let i = 0; i < 30; i++) {
          const timeoutMs = Math.min(2500 + i * 1000, 20000);
	        try {
	          const res = await fetchWithTimeout(hlsUrl, { cache: "no-store" }, timeoutMs);
	      ...
	      if (!ok) {
        setFatal("HLS playlist never became available (timeout).");
        return;
      }
    } catch (e: any) {
      setHlsStep("fail");
      setFatal(e?.message ?? "HLS check failed.");
      return;
    }
    ...
    try {
      ...
      if (!noHlsOverride) params.set("hls", announceHlsUrl);
      if (integrityMode) {
      ...
      <main className="relative max-w-7xl mx-auto p-6 space-y-6">
        ...
        <header className="space-y-2">
          ...
          <p className="text-sm text-neutral-300">
            ...
            {videoOnly ? " (video-only mode: chat/presence/P2P steps skipped)" : ""}
          </p>
