...
import { getVodCatalogEntry } from "@/lib/vodCatalog";
...
function normalizeRelativeVodPath(filePathSegments: string[]): string | null {
...
export async function GET(
  ...
  const authz = authorizeVodProxyRequest(normalized, accessToken, filePath ?? []);
  if (!authz.ok) {
    return new NextResponse(authz.error, { status: authz.status, headers: { "content-type": "text/plain; charset=utf-8" } });
  }

  const targetFile = resolveVodFile(normalized, filePath ?? []);
  if (!targetFile) return new NextResponse("Not found", { status: 404 });
  const relativePath = normalizeRelativeVodPath(filePath ?? []);
  if (!relativePath) return new NextResponse("Invalid file path", { status: 400 });

  const catalogEntry = getVodCatalogEntry(normalized, relativePath);
  if (!catalogEntry || !catalogEntry.publishedAtSec) {
    return new NextResponse("VOD is not available.", { status: 404 });
  }
  if (catalogEntry.processingState !== "ready") {
    return new NextResponse("VOD is still processing.", { status: 409 });
  }

  if (catalogEntry.visibility === "private") {
    if (!accessToken) return new NextResponse("VOD access denied.", { status: 403 });
    ...
    if (!verified.ok) return new NextResponse(`VOD access denied: ${verified.error}.`, { status: 403 });
    ...
    const hardDeny = decisions.some((decision) => decision.reasonCode === "deny_explicit" || decision.reasonCode === "deny_vod_archive_disabled");
    const allowed = !hardDeny && decisions.some((decision) => decision.allowed);
    if (!allowed) {
      return new NextResponse("VOD access denied.", { status: 403 });
    }
  }
