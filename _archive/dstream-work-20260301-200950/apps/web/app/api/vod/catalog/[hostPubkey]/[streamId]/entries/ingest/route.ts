...
import { type VodCatalogVisibility, type VodProcessingState } from "@/lib/vodCatalog";
...
function sanitizeVisibility(input: string | undefined): VodCatalogVisibility | undefined {
...
function sanitizeProcessingState(input: string | undefined): VodProcessingState | undefined {
  if (!input) return undefined;
  const value = input.trim().toLowerCase();
...
export async function POST(
  ...
  if (!originStreamId) {
    return Response.json({ ok: false, error: "Invalid stream id for VOD catalog ingest." }, { status: 400 });
  }
  ...
  const result = await syncVodCatalogEntriesFromFilesystem({
    originStreamId,
    onlyMissing,
    visibility,
    processingState,
