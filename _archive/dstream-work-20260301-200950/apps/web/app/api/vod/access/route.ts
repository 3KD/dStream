...
import { signVodAccessToken } from "@/lib/vod/accessToken";
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  const policy = resolveVodPolicy(announce);
  ...
  if (currency !== "xmr") return new Response("unsupported VOD currency", { status: 409 });
  ...
  if (requiredAtomic === null) return new Response("invalid VOD price policy", { status: 409 });
  ...
  try {
    ...
    const token = signVodAccessToken({
      v: 1,
      t: "dstream_vod_access",
    ...
    return new Response(`vod unlock error (${err?.message ?? "unknown"})`, {
      status: 502,
