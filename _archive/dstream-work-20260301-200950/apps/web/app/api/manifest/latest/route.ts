...
export async function GET(_req: NextRequest): Promise<Response> {
  try {
    const upstream = await fetch(`${MANIFEST_ORIGIN}/latest`, { cache: "no-store" });
    if (!upstream.ok) return new Response("manifest not available", { status: 404 });
    const text = await upstream.text();
    return new Response(text, { status: 200, headers: { "content-type": "application/json; charset=utf-8" } });
  } catch (err: any) {
    const message = `manifest latest error (${err?.message ?? "unknown"})`;
