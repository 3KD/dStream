import { disableVodAccessPackage } from "@/lib/access/packages";
...
export async function POST(req: Request): Promise<Response> {
  ...
  try {
    const pkg = disableVodAccessPackage({ packageId, hostPubkey });
    return Response.json({ ok: true, package: pkg, actorPubkey: auth.actorPubkey });
  } catch (error: any) {
    return Response.json({ ok: false, error: error?.message ?? "failed to disable VOD package" }, { status: 400 });
  }
}
