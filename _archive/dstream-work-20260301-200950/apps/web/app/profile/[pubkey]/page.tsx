...
export default function PublicProfilePage() {
  ...
  const vodStreams = useMemo(
    () => streams.filter((stream) => stream.status === "ended" && resolveVodPolicy(stream).mode !== "off"),
    [streams]
  );
  const vodGroups = useMemo(() => {
    const groups = new Map<string, StreamAnnounce[]>();
    for (const stream of vodStreams) {
      const playlistId = resolveVodPolicy(stream).playlistId?.trim() || "Ungrouped";
      const bucket = groups.get(playlistId) ?? [];
      bucket.push(stream);
      groups.set(playlistId, bucket);
    }
    return Array.from(groups.entries())
      .map(([playlistId, items]) => [playlistId, items.sort((a, b) => b.createdAt - a.createdAt)] as const)
      ...
      <main className="max-w-5xl mx-auto p-6 space-y-6">
        {!pubkey ? (
          ...
          <>
            ...
            <section className="rounded-2xl border border-neutral-800 bg-neutral-900/40 p-5 space-y-5">
              <div className="flex items-center justify-between">
                <h2 className="text-sm font-semibold text-neutral-200">Recent Streams</h2>
                <div className="flex items-center gap-3">
                  <Link href={`/vod?creator=${encodeURIComponent(npub ?? pubkey)}`} className="text-xs text-neutral-400 hover:text-white">
                    Creator VOD
                  </Link>
              ...
              {isLoadingStreams ? (
                ...
                <div className="space-y-5">
                  <div className="space-y-2">
                    <div className="text-xs text-neutral-500 uppercase tracking-wide">Live ({liveStreams.length})</div>
                    {liveStreams.length === 0 ? (
                      <div className="text-xs text-neutral-500 rounded-xl border border-neutral-800 bg-neutral-950/40 px-3 py-2">
                        No active lives announced recently.
                      </div>
                    ) : (
                      liveStreams.map((stream) => (
                        <div key={`live:${stream.pubkey}:${stream.streamId}`} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-xl border border-neutral-800 bg-neutral-950/40 px-3 py-2">
                          <div className="min-w-0">
                            <div className="text-sm text-neutral-200 truncate">{stream.title || stream.streamId}</div>
                            <div className="text-xs text-neutral-500">
                              LIVE · {new Date(stream.createdAt * 1000).toLocaleString()}
                            </div>
                          </div>
                          <Link
                  ...
                  <div className="space-y-2">
                    <div className="text-xs text-neutral-500 uppercase tracking-wide">Replays (VOD) ({vodStreams.length})</div>
                    {vodStreams.length === 0 ? (
                      <div className="text-xs text-neutral-500 rounded-xl border border-neutral-800 bg-neutral-950/40 px-3 py-2">
                        No replay announces available.
                      </div>
                    ) : (
                      <div className="space-y-3">
                        {vodGroups.map(([playlistId, items]) => (
                          <div key={playlistId} className="space-y-2">
                            <div className="text-[11px] text-neutral-500 uppercase tracking-wide">
                              Playlist: <span className="text-neutral-300 normal-case">{playlistId}</span> ({items.length})
                            </div>
                            {items.map((stream) => {
                              const vodPolicy = resolveVodPolicy(stream);
                              return (
                                <div key={`vod:${stream.pubkey}:${stream.streamId}:${stream.createdAt}`} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 rounded-xl border border-neutral-800 bg-neutral-950/40 px-3 py-2">
                                  <div className="min-w-0">
                                    <div className="text-sm text-neutral-200 truncate">{stream.title || stream.streamId}</div>
                                    <div className="text-xs text-neutral-500">
                                      {vodModeLabel(vodPolicy).toUpperCase()} · {new Date(stream.createdAt * 1000).toLocaleString()}
                                    </div>
                                    {vodPolicy.mode === "paid" && vodPolicy.priceAtomic && (
                                      <div className="text-xs text-amber-300 mt-1">Unlock: {formatXmrAtomic(vodPolicy.priceAtomic)}</div>
                                    )}
                                  </div>
                                  <Link
                                    href={`/watch/${npub ?? pubkey}/${stream.streamId}`}
