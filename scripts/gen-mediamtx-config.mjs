...
const disableInterfaceIpsLine = "webrtcIPsFromInterfaces: no";

let output = /^webrtcAdditionalHosts:/m.test(template)
  ? template.replace(/^webrtcAdditionalHosts:.*$/m, additionalHostsLine)
  : `${template.trimEnd()}\n${additionalHostsLine}\n`;

output = /^webrtcIPsFromInterfaces:/m.test(output)
  ? output.replace(/^webrtcIPsFromInterfaces:.*$/m, disableInterfaceIpsLine)
  : `${output.trimEnd()}\n${disableInterfaceIpsLine}\n`;
